...
export function drawLine2(data: any, index: number): void {
    ...
    for(let i = 0; i < end_index; i++) {
        if(points[i][0] === points[i + 1][0]) {
            ...
            forward_segs.push([points[i], points[i + 1], i, 0]);

        } else if(points[i][0] > points[i + 1][0] && points[i][1] < points[i + 1][1]) {
            forward_segs.push([points[i + 1], points[i], i, 1]);
    ...
    connectors.forEach((connector: LineConnect) => {
    ...
    // connectors.forEach((connector: LineConnect) => {
    //     let px = points[connector.pointIndex][0];
    //     connector.connectChar = "+";
    //     data['charMat'][connector.pointIndex]
